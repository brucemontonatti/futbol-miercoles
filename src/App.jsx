...
function buildTwoOptions(selectedIds, players, stats) {
  ...
  // Detectar arqueros por cualquier posicion (primaria o secundaria)
  const arqueros = selected.filter(p => p.positions.includes("Arquero"));
  const campo    = selected.filter(p => !p.positions.includes("Arquero"));

  // Garantizar al menos 2 arqueros tomando jugadores de campo si hace falta
  const arqPool   = [...arqueros];
  const campoPool = [...campo];
  while (arqPool.length < 2 && campoPool.length > 0) arqPool.push(campoPool.shift());

  // Ordenar campo por ranking (pts desc)
  const ranked = [...campoPool].sort((a,b) => pts(stats[b.id]||{}) - pts(stats[a.id]||{}));

  const makeBalancedTeams = () => {
    // Shuffle arqueros y asignar UNO a cada equipo - siempre separados
    const shuffledArqs = shuffle(arqPool);
    let negro  = [{ ...shuffledArqs[0], role:"Arquero" }];
    let blanco = [{ ...shuffledArqs[1], role:"Arquero" }];

    // Arqueros extra (si hubiera 3+) van al campo con posicion secundaria
    const extraArqs = shuffledArqs.slice(2).map(p => ({
      ...p, role: p.positions.find(pos => pos !== "Arquero") || "Defensor"
    }));

    // Snake draft con el resto del campo
  ...
  // Verificar que cada opcion tenga arqueros separados
